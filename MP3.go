...
func average(currentStates map[int]float32) float32 {
	total := float32(0)

	//averages recieved states
	for _, states := range currentStates {
		total = total + states
	}
	return (total / float32(len(currentStates)))
}
